...
async function nMember(m, lid){
  showSaving(true);
  const r = await sbInsert('team', {name:m.name,role:m.role,dept:m.dept,access:m.access||'Member',status:'Active',email:m.email||'',wa:m.wa||'',wa_apikey:m.wa_apikey||'',color:m.color,av:m.av,username:m.username||m.name.toLowerCase().split(' ')[0],password:m.password||'abohamood@1.',member_type:m.memberType||''});
  showSaving(false);
  if(r?.id) m.id=r.id;
  return r;
}
async function nMemberUpd(m){
  showSaving(true);
  const r = await sbUpdate('team', m.id, {name:m.name,role:m.role,dept:m.dept,access:m.access,email:m.email||'',wa:m.wa||'',wa_apikey:m.wa_apikey||'',username:m.username||m.name.toLowerCase().split(' ')[0],password:m.password||'abohamood@1.',member_type:m.memberType||''});
