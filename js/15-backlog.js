...
window.openBacklog=(id)=>{
  ...
  if(isAdmin()){
    body+=`<div class="spa">
      ...
      <button class="btn bd2 bsm" onclick="delItem('backlog','${b.id}','${b.title.replace(/'/g,"\\'")}');closeSP()">🗑 Delete</button>
    </div>
...
window.convertBL=(id)=>{
  ...
  nCreateTask(t,t.id);
  logAction('Backlog Converted',`"${b.title}" converted to task`,'Success',b.title,'');
